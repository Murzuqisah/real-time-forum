...
func CreatePost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println("Invalid Session on cookie")
		log.Println("cookie", cookie)
		log.Println("SessionStore", SessionStore)
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(map[string]string{
	...
	sessionData, ok := SessionStore[cookie]

	if !ok {
		log.Println("Invalid Session on stored sessions")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(map[string]string{
			"error": "Invalid Session",
		})
	...
	id, err := repositories.InsertRecord(util.DB, "tblPosts", []string{"post_title", "body", "media_url", "user_id"}, html.EscapeString(r.FormValue("post-title")), html.EscapeString(r.FormValue("post-content")), url, sessionData)
	if err != nil {
